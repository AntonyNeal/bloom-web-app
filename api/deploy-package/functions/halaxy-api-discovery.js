...
async function testEndpoint(baseUrl, endpoint, token, params) {
    try {
...
async function halaxyApiDiscoveryHandler(req, context) {
    const headers = {
    ...
    try {
        ...
        const { getAccessToken, getHalaxyConfig } = await Promise.resolve().then(() => __importStar(require('../services/halaxy/token-manager')));
...
function generateRecommendations(available, unavailable) {
    ...
    if (!hasSlot && !hasSchedule) {
        recommendations.push('No Slot or Schedule endpoint available. Consider generating availability from practitioner working hours or gaps in appointments.');
    }
    if (hasAppointment && !hasSlot) {
        recommendations.push('Appointment endpoint available but not Slot. Could calculate free slots by finding gaps in booked appointments.');
    }
    if (hasPatient) {
        recommendations.push('Patient endpoint available. Try fetching all patients without practitioner filter, then filter client-side.');
    }
    if (hasAppointment) {
        recommendations.push('Appointment endpoint available. Try fetching appointments by date range only, without practitioner filter.');
    }
    return recommendations;
}
functions_1.app.http('halaxyApiDiscovery', {
    methods: ['GET'],
    authLevel: 'anonymous',
    route: 'halaxy/discover',
    handler: halaxyApiDiscoveryHandler,
});
exports.default = halaxyApiDiscoveryHandler;
//# sourceMappingURL=halaxy-api-discovery.js.map