...
export function criticalCssInline() {
    ...
    return {
        ...
        buildStart() {
            ...
            try {
                ...
                console.log(`[CRITICAL CSS] Inlined ${criticalCss.length} characters of critical CSS`);
            }
            catch {
                console.warn('Critical CSS file not found at', criticalCssPath, 'skipping inlining');
            }
        },
    ...
    <style>${criticalCss}</style>`);
            ...
            modifiedHtml = modifiedHtml.replace(/<link[^>]*rel="stylesheet"[^>]*href="([^"]*index-[^"]*\.css)"[^>]*>/g, (_match, href) => {
                return `<script>
            (function() {
        ...
        generateBundle(_options, bundle) {
            ...
            for (const [fileName, chunk] of Object.entries(bundle)) {
                if (fileName.endsWith('.css') && chunk.type === 'asset') {
                    ...
                    if (cssContent.includes('/* Critical CSS for above-the-fold content */')) {
                        // Split by the critical CSS marker and keep only the non-critical part
                        const parts = cssContent.split('/* Critical CSS for above-the-fold content - Life Psychology Australia */');
                        if (parts.length > 1) {
                            // Keep everything after the critical CSS
                            chunk.source = parts
                                .slice(1)
                                .join('/* Critical CSS for above-the-fold content - Life Psychology Australia */');
                        }
                    }
                }
            }
