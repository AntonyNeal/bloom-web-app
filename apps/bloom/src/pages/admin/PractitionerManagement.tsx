...
export function PractitionerManagement() {
  ...
  const toggleActive = async (practitioner: Practitioner) => {
    ...
    try {
      ...
      toast({
        title: practitioner.is_active ? '≡ƒöÆ Practitioner Deactivated' : 'Γ£à Practitioner Activated',
  ...
  const getStatusBadge = (practitioner: Practitioner) => {
    if (!practitioner.onboarding_completed_at) {
      return <Badge className="bg-yellow-100 text-yellow-800">ΓÅ│ Onboarding Pending</Badge>;
    }
    if (practitioner.is_active) {
      return <Badge className="bg-emerald-100 text-emerald-800">Γ£à Active</Badge>;
    }
    return <Badge className="bg-neutral-100 text-neutral-600">≡ƒöÆ Inactive</Badge>;
  };
                        ...
                        <h3 className="font-medium text-neutral-800">
                          ...
                          {practitioner.favorite_flower && (
                            <span className="ml-2 text-sm" title={`Favorite flower: ${practitioner.favorite_flower}`}>
                              {practitioner.favorite_flower === 'Cherry Blossom' && '≡ƒî╕'}
                              {practitioner.favorite_flower === 'Purple Rose' && '≡ƒî╣'}
                              {practitioner.favorite_flower === 'Sunflower' && '≡ƒî╗'}
                              {!['Cherry Blossom', 'Purple Rose', 'Sunflower'].includes(practitioner.favorite_flower) && '≡ƒî╝'}
                            </span>
                          )}
                        </h3>
