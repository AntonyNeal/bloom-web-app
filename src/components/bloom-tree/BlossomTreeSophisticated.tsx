...
export const BlossomTreeSophisticated: React.FC<BlossomTreeProps> = ({
  ...
  const getGrowthStage = (pct: number) => {
    if (pct < 15) return { name: 'dormant', label: '🌱 Dormant', blossomCount: 0, intensity: 0.3 };
    if (pct < 30) return { name: 'budding', label: '🌿 Early Buds', blossomCount: 12, intensity: 0.4 };
    if (pct < 50) return { name: 'emerging', label: '🌸 Emerging', blossomCount: 28, intensity: 0.55 };
    if (pct < 70) return { name: 'blooming', label: '🌺 Blooming', blossomCount: 52, intensity: 0.7 };
    if (pct < 90) return { name: 'flourishing', label: '🌸🌸 Flourishing', blossomCount: 84, intensity: 0.85 };
    return { name: 'fullBloom', label: '🌳 Peak Bloom', blossomCount: 120, intensity: 1.0 };
  };
