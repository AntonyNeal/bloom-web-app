...
-- Drop old Halaxy-based clients table if it has old schema
-- (Old schema has 'presenting_issues' column, new schema has 'is_deleted')
IF EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'clients')
   AND EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'clients' AND COLUMN_NAME = 'presenting_issues')
BEGIN
    PRINT 'V035: Dropping old Halaxy-based clients table to recreate with new schema';
    DROP TABLE clients;
END
GO

-- Only create if table doesn't exist (handles fresh installs and post-drop)
IF NOT EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'clients')
BEGIN
    PRINT 'V035: Creating clients table with new standalone schema';
...
ELSE
BEGIN
    PRINT 'V035: Clients table already exists with correct schema, skipping creation';
END
GO

-- Create indexes (idempotent - checks if exists)
...
BEGIN
    ...
    PRINT 'V035: Created IX_clients_practitioner index';
END
GO
...
BEGIN
    ...
    PRINT 'V035: Created IX_clients_email index';
END
GO
...
BEGIN
    ...
    PRINT 'V035: Created IX_clients_phone index';
END
GO
...
BEGIN
    ...
    PRINT 'V035: Created IX_clients_halaxy_id index';
END
GO
...
BEGIN
    CREATE INDEX IX_clients_name 
        ...
        WHERE is_deleted = 0;
    PRINT 'V035: Created IX_clients_name index';
END
GO

PRINT 'V035: Migration complete';
