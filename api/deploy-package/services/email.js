...
exports.sendClinicianBookingNotification = sendClinicianBookingNotification;
 ...
 */
async function sendEmail(to, subject, htmlContent, plainTextContent) {
 ...
 */
async function sendDenialEmail(context) {
...
The ${BLOOM_NAME} Team
  `.trim();
    return sendEmail(email, `Your ${BLOOM_NAME} Application`, htmlContent, plainTextContent);
}
/**
 * Waitlist Email Template
 */
async function sendWaitlistEmail(context) {
    const { firstName, email } = context;
    const htmlContent = wrapInTemplate(`
...
The ${BLOOM_NAME} Team
  `.trim();
    return sendEmail(email, `Your ${BLOOM_NAME} Application - On Waitlist`, htmlContent, plainTextContent);
}
/**
 ...
 */
async function sendInterviewEmail(context) {
...
Zoe & The ${BLOOM_NAME} Team
  `.trim();
    return sendEmail(email, `${BLOOM_NAME} - Schedule Your Interview 📅`, htmlContent, plainTextContent);
}
/**
 * Acceptance/Onboarding Email Template
 */
async function sendAcceptanceEmail(context) {
...
Zoe & The ${BLOOM_NAME} Team
  `.trim();
    return sendEmail(email, `Welcome to ${BLOOM_NAME}! 🎉`, htmlContent, plainTextContent);
}
/**
 * Send email notification to clinician when a new booking is made
 */
async function sendClinicianBookingNotification(context) {
    const { practitionerEmail, practitionerFirstName, patientFirstName, patientLastName, patientEmail, patientPhone, appointmentDateTime, appointmentType, } = context;
...
The ${BLOOM_NAME} Team
  `.trim();
    return sendEmail(practitionerEmail, `📅 New Booking: ${patientFirstName} ${patientLastName} - ${formattedTime}`, htmlContent, plainTextContent);
}
