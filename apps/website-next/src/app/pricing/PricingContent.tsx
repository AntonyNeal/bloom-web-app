...
const faqs = [
  ...
  },
  {
    ...
    answer:
      'As an NDIS provider, we offer psychology sessions at $232.99 per session (NDIS price guide rate) for participants with self-managed or plan-managed funding. This allows you to use your NDIS psychology budget directly. Contact us to confirm your NDIS management type before booking.',
  },
  {
    question: 'What are Professional Supervision sessions?',
    answer:
      'Professional Supervision sessions are exclusively for psychology students, provisional psychologists, or registered psychologists who require clinical supervision for AHPRA registration requirements. These are NOT therapy sessions for personal mental health issues.',
  },
  {
...
export default function PricingContent() {
  ...
  const fundingOptions = allFundingOptions.filter((option) => {
    if (option.value === 'medicare' && sessionType === 'couples') {
      return false;
    }
    return true;
  });

  const getRebateLabel = (funding: string): string => {
    switch (funding) {
      case 'medicare':
  ...
  const formatRebateAmount = (funding: string, rebateAmount: number): string => {
    if (funding === 'private') {
  ...
  const formatOutOfPocketAmount = (funding: string, outOfPocket: number, rebateAmount: number): string => {
    if (funding === 'private') {
  ...
  const calculateOutOfPocket = () => {
    ...
    if (fundingType === 'ndis') {
      return { outOfPocket: 0, rebate: basePrice, description: 'Fully covered by NDIS funding' };
    }

    if (fundingType === 'medicare') {
      if (sessionType === 'couples') {
        return { outOfPocket: basePrice, rebate: 0, description: 'Medicare does not cover couples therapy' };
      }
      const medicareRebate = 98.95;
      return { outOfPocket: basePrice - medicareRebate, rebate: medicareRebate, description: 'With Medicare rebate (GP referral required)' };
    }

    if (fundingType === 'private') {
      const typicalRebate = Math.min(basePrice * 0.6, 100);
      return { outOfPocket: basePrice - typicalRebate, rebate: typicalRebate, description: 'ESTIMATE ONLY - Contact your insurer to confirm' };
    }

    return { outOfPocket: basePrice, rebate: 0, description: 'Full private payment' };
  };

  const getSessionPrice = (type: string) => {
    const prices = { individual: '250.00', couples: '300.00', ndis: '232.99' };
    return prices[type as keyof typeof prices] || '250.00';
  };

  const getRebateExplanation = (funding: string, rebateAmount: number): string => {
    switch (funding) {
      case 'medicare':
      ...
      case 'private': {
        const minRebate = Math.max(0, rebateAmount - 30);
        const maxRebate = rebateAmount + 20;
        return `⚠️ Private health rebate typically ranges A$${minRebate.toFixed(2)} - A$${maxRebate.toFixed(2)} depending on your policy level, provider, and annual limits. Contact your insurer directly before booking to confirm your exact coverage.`;
      }
      case 'ndis':
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-orange-50/10">
      ...
      <section className="relative bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white py-16 lg:py-20 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <p className="text-lg mb-8 text-blue-50">No waiting lists • Start today • Secure online sessions</p>
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 lg:p-10">
                ...
                {[1, 2, 3].map((stepNum) => (
                  <div
                    ...
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold ${
                      stepNum <= calculatorStep ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-500'
                    }`}
                  >
                    {stepNum}
                  </div>
            ...
            {calculatorStep === 0 && (
              <div className="text-center space-y-6">
                <h3 className="text-xl font-semibold text-gray-900">Let&apos;s find your personalised cost</h3>
                <p className="text-gray-600">Answer a few quick questions to see exactly what you&apos;ll pay</p>
                <button
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (option.value === 'ndis') {
                          setFundingType('ndis');
                          setCalculatorStep(3);
                        } else if (option.value === 'couples' && fundingType === 'medicare') {
                      ...
                      className="p-6 border-2 border-gray-200 rounded-lg hover:border-blue-500 hover:bg-blue-50 text-left transition-all duration-200"
                    >
                ...
                {sessionType === 'couples' && (
                  <div className="p-4 bg-orange-50 border border-orange-200 rounded-lg">
                    <p className="text-orange-800 text-sm">
                      ℹ️ <strong>Note:</strong> Medicare rebates only apply to individual psychology sessions. Couples therapy is not covered by Medicare, but may be covered by some private health funds.
                    </p>
                  </div>
                )}
                  ...
                  {fundingOptions
                    ...
                    .map((option) => (
                      ...
                      >
                        ...
                        {option.badge && (
                          <span className="inline-block px-2 py-1 bg-green-100 text-green-800 text-xs font-semibold rounded mt-2">{option.badge}</span>
                        )}
                      </button>
                    ))}
                </div>
              ...
              <div className="text-center space-y-6">
                <div className="bg-gradient-to-br from-green-50 to-blue-50 rounded-xl p-8">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center text-lg">
                      ...
                      <span className="font-semibold">A${getSessionPrice(sessionType)}</span>
                    </div>

                    {result.rebate > 0 && (
                      <div className="flex justify-between items-center text-lg text-green-600">
                        <span>{getRebateLabel(fundingType)}</span>
                        <span className="font-bold">{formatRebateAmount(fundingType, result.rebate)}</span>
                      </div>
                    )}

                    <div className="border-t border-gray-200 pt-4">
                      <div className="flex justify-between items-center">
                        <span className="text-xl font-semibold text-gray-900">You are out-of-pocket:</span>
                        <span className="text-3xl font-bold text-blue-600">{formatOutOfPocketAmount(fundingType, result.outOfPocket, result.rebate)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  {result.rebate > 0 && (
                    <div className="mt-6 p-4 bg-green-100 border border-green-200 rounded-lg">
                      <p className="text-green-800 font-semibold">{getRebateExplanation(fundingType, result.rebate)}</p>
                    </div>
                  )}
                </div>
                ...
                {fundingType === 'no-coverage' && (
                  <div className="mt-6 p-4 bg-orange-50 border border-orange-200 rounded-lg">
                    <h4 className="font-semibold text-orange-800 mb-2">💡 Available Funding Options:</h4>
                    <ul className="text-orange-700 text-sm space-y-1 text-left">
                      {sessionType !== 'couples' && <li>• Check if you&apos;re eligible for Medicare rebate with a GP referral</li>}
                      <li>• Consider private health insurance with psychology extras coverage</li>
                      <li>• Some employers offer Employee Assistance Programs (EAP)</li>
                      {sessionType === 'couples' && (
                        <li>• Alternative: Book individual sessions (Medicare eligible at A$151.05 each with GP referral)</li>
                      )}
                    </ul>
                  </div>
                )}
                ...
                <div className="space-y-4">
                  ...
                  >
                    {fundingType === 'no-coverage' ? 'Book Session → A$250.00' : `Book at This Cost → A$${result.outOfPocket.toFixed(2)}`}
                  </button>
      ...
      <section className="py-16 lg:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-4">Appointment Types Available</h2>
            <p className="text-xl text-gray-600">Different session types for different therapeutic needs</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-16">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <div className="mb-4">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Individual Psychology Session</h3>
                <div className="text-2xl font-bold text-blue-600 mb-2">A$250.00</div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <div className="mb-4">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Couples Session</h3>
                <div className="text-2xl font-bold text-purple-600 mb-2">A$300.00</div>
                <p className="text-sm text-gray-500">60 minutes</p>
              </div>
              <ul className="space-y-2 text-sm text-gray-700">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <div className="mb-4">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">NDIS Psychology Session</h3>
                <div className="text-2xl font-bold text-green-600 mb-2">A$232.99</div>
                <p className="text-sm text-gray-500">60 minutes</p>
              </div>
              <ul className="space-y-2 text-sm text-gray-700">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <div className="mb-4">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Medicare Psychology Session</h3>
                <div className="text-2xl font-bold text-blue-600 mb-2">A$250.00</div>
                <p className="text-sm text-gray-500">60 minutes (with rebate)</p>
              </div>
              <ul className="space-y-2 text-sm text-gray-700">
            ...
            <div className="bg-orange-50 border border-orange-200 rounded-xl p-6 shadow-sm">
              <div className="mb-4">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Professional Supervision</h3>
                <div className="text-2xl font-bold text-orange-600 mb-2">A$250.00</div>
                <p className="text-sm text-gray-500">60 minutes</p>
              ...
              <ul className="space-y-2 text-sm text-gray-700">
                <li>• For psychology students or provisionally registered psychologists seeking supervision</li>
          ...
          <div className="text-center mb-12">
            <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-4">Funding Options Available</h2>
            <p className="text-xl text-gray-600">Different ways to fund your psychology sessions</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <div className="mb-4">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Medicare Benefits</h3>
                <div className="text-2xl font-bold text-blue-600 mb-2">A$98.95 rebate</div>
                <p className="text-sm text-gray-500">Registered Psychologist rebate (Item 80010)</p>
              </div>
              ...
              <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded">
                <strong>Your cost:</strong> A$250.00 - A$98.95 = <strong>A$151.05</strong>
              </div>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <div className="mb-4">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Private Health Insurance</h3>
                <div className="text-2xl font-bold text-purple-600 mb-2">Varies</div>
                <p className="text-sm text-gray-500">Depends on your level of cover</p>
              </div>

              <div className="space-y-3 mb-4">
                <p className="text-sm text-gray-700 leading-relaxed">
                  Many private health policies provide rebates for psychology services, but coverage varies significantly.
                </p>

                <div className="bg-blue-50 border border-blue-200 rounded p-3">
                  <p className="text-xs font-medium text-blue-800 mb-2">To check your rebate, contact your insurer with:</p>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <div className="mb-4">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">NDIS Funding</h3>
                <div className="text-2xl font-bold text-green-600 mb-2">Fully funded</div>
                <p className="text-sm text-gray-500">For eligible participants</p>
              </div>
              <ul className="space-y-2 text-sm text-gray-700 mb-4">
                <li>• NDIS provider - direct billing available</li>
              ...
              <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded">
                <strong>Your cost:</strong> <strong>A$0.00</strong> (if you have NDIS funding)
              </div>
            </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <div className="mb-4">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Private Pay</h3>
                <div className="text-2xl font-bold text-gray-700 mb-2">Full fee</div>
              ...
              <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded">
                <strong>Your cost:</strong> Full session fee (A$250.00-A$300.00)
              </div>
            </div>
          </div>
          ...
          <div className="text-center mt-12">
            ...
            <p className="mt-4 text-gray-600">Choose your appointment type and funding option during booking</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 lg:py-24 bg-gray-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-4">Your Peace of Mind is Our Priority</h2>
            <p className="text-xl text-gray-600">Professional, secure, and confidential mental health support</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Professional Credentials</h3>
              <p className="text-gray-600">Registered Psychologist with AHPRA</p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600">Professional healthcare privacy standards</p>
            </div>
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600">Online booking available through our secure client portal</p>
            </div>
            ...
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">Personalised Care</h3>
              <p className="text-gray-600">Individual approach tailored to your needs</p>
            </div>
          </div>

          {/* Risk Mitigation */}
          <div className="bg-white rounded-2xl shadow-lg p-8">
            <h3 className="text-2xl font-bold text-gray-900 text-center mb-8">Service Standards</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <div className="text-center">
                ...
                <h4 className="text-lg font-semibold text-gray-900 mb-2">Clear Pricing</h4>
                <p className="text-gray-600">Upfront costs with no surprise fees</p>
              </div>
              ...
              <div className="text-center">
                ...
                <h4 className="text-lg font-semibold text-gray-900 mb-2">Transparent Billing</h4>
              ...
              <div className="text-center">
                ...
                <p className="text-gray-600">Evidence-based therapy approaches</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 lg:py-24 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <h2 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-4">Common Questions About Pricing</h2>
            <p className="text-xl text-gray-600">Get clear answers about costs and funding options</p>
          </div>
            ...
            {faqs.map((faq, index) => (
              <div key={index} className="border border-gray-200 rounded-lg">
                ...
                >
                  ...
                  <span className={`text-2xl transition-transform duration-200 ${openFAQ === index ? 'rotate-45' : ''}`}>+</span>
                </button>
      ...
      <section className="py-16 lg:py-24 bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl mb-8 text-blue-100">Professional mental health support when you need it most</p>
          ...
          <div className="bg-blue-700 bg-opacity-50 rounded-lg p-4 mb-6">
            <p className="text-blue-100 text-sm">
              <strong>Secure booking</strong> • <strong>No commitment required</strong> • <strong>Available 6 days a week</strong>
            </p>
          </div>

          <p className="text-blue-100 text-sm">Not sure which option is right for you? Contact us and we&apos;ll help you choose.</p>
        </div>
      </section>
    </div>
  );
