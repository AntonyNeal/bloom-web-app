...
class MigrationService {
    ...
    async connect() {
        ...
        if (!this.cosmosClient && (0, config_1.isCosmosConfigured)()) {
            const cosmosConfig = (0, config_1.getCosmosConfig)();
            if (cosmosConfig) {
                try {
                    this.cosmosClient = new cosmos_1.CosmosClient(cosmosConfig.connectionString);
                    ...
                    console.warn('⚠️  Cosmos DB connection failed, running in SQL-only mode:', error.message);
                    this.cosmosEnabled = false;
                }
            }
        }
...
`;
    ...
    async captureSchemaSnapshot(input) {
        ...
        if (container) {
            ...
            cosmosDocumentId = resource === null || resource === void 0 ? void 0 : resource.id;
        }
