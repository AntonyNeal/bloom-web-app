...
const TEST_CONFIGURATIONS = {
    ...
    'hero-cta-test': {
        ...
        description: 'Single CTA vs Multiple CTAs - Research shows 371% more clicks',
 ...
 */
function allocateUserToVariant(userId, testConfig) {
 ...
 */
async function trackVariantAllocation(userId, testName, variant, deviceType, location) {
    ...
    try {
        ...
        console.error('Error tracking variant allocation:', error);
    }
}
...
app.http('allocateVariant', {
    ...
    handler: async (request, context) => {
        ...
        try {
            // Extract parameters
            const testName = request.query.get('test') || 'homepage-header-test';
            const userId = request.query.get('userId') || generateUserId(request);
            ...
            // Track allocation
            await trackVariantAllocation(userId, testName, variant, deviceType, location);
            ...
            context.log(`Allocated user ${userId} to variant ${variant} for test ${testName}`);
            ...
            context.log.error('Error in variant allocation:', error);
