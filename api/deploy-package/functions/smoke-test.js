...
async function testApiEndpoints(baseUrl) {
    ...
    results.push(await runTest('API Health Check', async () => {
    ...
    results.push(await runTest('Applications API (GET)', async () => {
    ...
    results.push(await runTest('A/B Test API', async () => {
    ...
    if (!cosmosConnectionString) {
        ...
        results.push(await runTest('DBVC Health API', async () => {
...
async function testSqlDatabase() {
    ...
    let pool = null;
    // Test connection
    results.push(await runTest('SQL Connection', async () => {
    ...
    for (const table of tables) {
        results.push(await runTest(`SQL Table: ${table.name}`, async () => {
            const subTests = [];
        ...
        `);
            ...
            const allColumnsExist = table.expectedColumns.every((col) => existingColumns.includes(col.toLowerCase()));
            return {
...
// COSMOS DB TESTS
// ============================================================================
async function testCosmosDatabase() {
    const results = [];
    ...
    // Test connection
    results.push(await runTest('Cosmos Connection', async () => {
    ...
    for (const containerName of containers) {
        results.push(await runTest(`Cosmos Container: ${containerName}`, async () => {
            try {
                const client = new cosmos_1.CosmosClient(connectionString);
...
async function testSchemaIntegrity() {
    ...
    try {
        ...
        pool = await sql.connect(config);
        // Test indexes exist
        results.push(await runTest('Database Indexes', async () => {
        ...
        `);
            ...
            const subTests = indexResult.recordset.slice(0, 10).map((idx) => ({
        ...
        // Test foreign keys
        results.push(await runTest('Foreign Key Constraints', async () => {
        ...
        results.push(await runTest('Stored Procedures', async () => {
            const spResult = await pool.request().query(`
          SELECT name, create_date, modify_date
          FROM sys.procedures
          ORDER BY name
        `);
            const subTests = spResult.recordset.map((sp) => ({
                name: sp.name,
                status: 'pass',
                duration: 0,
...
// MAIN HANDLER
// ============================================================================
async function smokeTestHandler(req, _context) {
...
functions_1.app.http('smoke-test', {
    methods: ['GET', 'OPTIONS'],
    authLevel: 'anonymous',
    route: 'smoke-test',
    handler: smokeTestHandler,
});
exports.default = smokeTestHandler;
